	...
	protected void calcDiscounts(int[][] fofs) {
		...
		for(int i = (smoothUnigrams?0:1); i < newdisc.length; i++) {
			...
			for(int j = 0; j < FREQ_CUTOFF; j++) {
                ...
                if(fofs[i][j+1] == 0) {
                    System.err.println("WARNING: no "+(i+1)+"-grams of frequency "+(j+2)+". Modified Kneser-Ney will not work.");
                } else if (fofs[i][j] == 0) {
                    System.err.println("WARNING: no "+(i+1)+"-grams of frequency "+(j+1)+". Modified Kneser-Ney will not work.");
                } else if (Y != 0) {
